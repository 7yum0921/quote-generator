            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>報價單編號</label>
                        <input type="text" id="quoteNumber" placeholder="Q001" value="Q001">
                    </div>
                    <div class="form-group">
                        <label>日期</label>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>乙方（服務提供者）</label>
                        <input type="text" id="providerName" placeholder="公司/個人名稱">
                    </div>
                    <div class="form-group">
                        <label>甲方（客戶）</label>
                        <input type="text" id="clientName" placeholder="客戶名稱">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>乙方聯絡方式</label>
                        <input type="text" id="providerContact" placeholder="電話/Email">
                    </div>
                    <div class="form-group">
                        <label>甲方聯絡方式</label>
                        <input type="text" id="clientContact" placeholder="電話/Email">
                    </div>
                </div>
                
                <div class="items-section">
                    <h3>服務項目</h3>
                    <button type="button" class="add-item-btn" onclick="addItem()">+ 新增項目</button>
                ...
                <div class="form-group">
                    <label>備註</label>
                    <textarea id="notes" rows="4" placeholder="其他說明或條件"></textarea>
                </div>
                
                <div class="signature-section">
                    <div>
                        <label>乙方簽名</label>
                        <div class="signature-box" onclick="startSigning('provider')">
                            <canvas id="providerSignature" class="signature-canvas" width="200" height="120"></canvas>
                            <span id="providerPlaceholder">點擊簽名</span>
                        </div>
                        <div class="signature-controls">
                            <button type="button" class="clear-signature" onclick="clearSignature('provider')">清除</button>
                        </div>
                    </div>
                    
                    <div>
                        <label>甲方簽名</label>
                        <div class="signature-box" onclick="startSigning('client')">
                            <canvas id="clientSignature" class="signature-canvas" width="200" height="120"></canvas>
                            <span id="clientPlaceholder">點擊簽名</span>
                        </div>
                        <div class="signature-controls">
                            <button type="button" class="clear-signature" onclick="clearSignature('client')">清除</button>
                        </div>
                    </div>
                </div>
                
                <div class="action-buttons">
                    <button type="button" class="btn btn-secondary" onclick="shareQuote()">分享連結</button>
                    <button type="button" class="btn btn-primary" onclick="downloadPDF()">下載PDF</button>
                </div>
            </div>
            ...
            <div class="preview-section">
                <div class="quote-preview" id="quotePreview">
                    <div class="quote-header">
                        <h2>報 價 單</h2>
                        <p>Quote No: <span id="previewQuoteNumber">Q001</span> | Date: <span id="previewDate"></span></p>
                    </div>
                    
                    <div class="quote-info">
                        <div class="info-section">
                            <h4>乙方（服務提供者）</h4>
                            <p><strong>名稱：</strong><span id="previewProviderName">-</span></p>
                            <p><strong>聯絡方式：</strong><span id="previewProviderContact">-</span></p>
                        </div>
                        <div class="info-section">
                            <h4>甲方（客戶）</h4>
                            <p><strong>名稱：</strong><span id="previewClientName">-</span></p>
                            <p><strong>聯絡方式：</strong><span id="previewClientContact">-</span></p>
                        </div>
                    </div>
                    
                    <div class="project-name">
                        <h4>專案名稱：<span id="previewProjectName">-</span></h4>
                    </div>
                    <br>
                    ...
                    <div class="signature-preview">
                        <div class="signature-area">
                            <div id="providerSignaturePreview"></div>
                            <div class="signature-line">乙方簽名</div>
                        </div>
                        <div class="signature-area">
                            <div id="clientSignaturePreview"></div>
                            <div class="signature-line">甲方簽名</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function shareQuote() {
            ...
            if (!isCanvasEmpty(providerCanvas)) {
                quoteData.providerSignature = providerCanvas.toDataURL();
            }
            if (!isCanvasEmpty(clientCanvas)) {
                quoteData.clientSignature = clientCanvas.toDataURL();
            }
            
            try {
                const encodedData = btoa(encodeURIComponent(JSON.stringify(quoteData)));
                const shareUrl = `${window.location.origin}${window.location.pathname}?view=client&data=${encodedData}`;
                
                if (navigator.share) {
                    navigator.share({
                        title: `報價單 - ${quoteData.projectName || '專案'}`,
                        url: shareUrl
                    }).catch(err => {
                        // 如果分享失敗，則複製到剪貼板
        ...
        window.addEventListener('load', function() {
            ...
            if (data) {
                try {
                    const quoteData = JSON.parse(decodeURIComponent(atob(data)));
        ...
        function setupClientView(quoteData) {
            ...
            if (quoteData.providerSignature) {
                document.getElementById('providerSignatureDisplay').innerHTML = 
                    `<img src="${quoteData.providerSignature}" style="max-width: 150px; max-height: 60px; border: 1px solid #d1d5db; border-radius: 4px;">`;
            }
        }
        ...
        function downloadSignedPDF() {
            ...
            html2canvas(previewElement, {
                ...
                if (data) {
                    try {
                        const quoteData = JSON.parse(decodeURIComponent(atob(data)));
                        projectName = quoteData.projectName || '專案';
                        quoteNumber = quoteData.quoteNumber || 'Q001';
                    } catch (e) {
        ...
        function loadQuoteData(quoteData) {
            ...
            if (quoteData.items && quoteData.items.length > 0) {
                ...
                quoteData.items.forEach(item => {
                    ...
                    itemRow.innerHTML = `
                        <div class="form-group">
                            <label>項目名稱</label>
                            <input type="text" class="item-name" value="${item.name || ''}" placeholder="服務項目">
                        </div>
                        <div class="form-group">
                            <label>數量</label>
                        ...
                        <div class="form-group">
                            <label>單價</label>
                            <input type="number" class="item-price" value="${item.price || 0}" placeholder="0">
                        </div>
                        <div class="form-group">
                            <label>金額</label>
                            <input type="number" class="item-amount" readonly>
            ...
            if (quoteData.providerSignature) {
                // 這裡可以載入乙方簽名到canvas
                // 由於canvas載入圖片比較複雜，這裡先更新預覽
                document.getElementById('providerSignaturePreview').innerHTML = 
                    `<img src="${quoteData.providerSignature}" style="max-width: 150px; max-height: 60px;">`;
            }
            
            if (quoteData.clientSignature) {
                document.getElementById('clientSignaturePreview').innerHTML = 
                    `<img src="${quoteData.clientSignature}" style="max-width: 150px; max-height: 60px;">`;
            }
